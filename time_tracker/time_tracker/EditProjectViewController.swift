...
class EditProjectViewController: UIViewController {
	...
	var taskEditName: ProjectsList? {
		didSet {
			nameProjectTextFiled.text = taskEditName!.name
			redColor = taskEditName?.red ?? 0.94
			greenColor = taskEditName?.green ?? 0.12
			blueColor = taskEditName?.blue ?? 0.35
		}
	}
	...
	@objc private func buttonClicked() {
		...
		self.taskEditName?.name = text
		self.taskEditName?.red = self.redColor
		self.taskEditName?.green = self.greenColor
		self.taskEditName?.blue = self.blueColor
		do{
			try context.save()
